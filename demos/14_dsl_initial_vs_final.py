...
def pretty_print(expr) -> str:
    match expr:
        ...
        case Mul(a, b): return f"({pretty_print(a)} * {pretty_print(b)})"
        case Neg(a): return f"(-{pretty_print(a)})"

def count_ops(expr) -> int:
    match expr:
        case Lit(_): return 0
        case Add(a, b): return 1 + count_ops(a) + count_ops(b)
        case Mul(a, b): return 1 + count_ops(a) + count_ops(b)
        case Neg(a): return 1 + count_ops(a)

def optimize(expr):
    """Optimization pass: simplify double negation, multiply by 0/1."""
    match expr:
        case Neg(Neg(inner)): return optimize(inner)
        case Mul(Lit(0), _) | Mul(_, Lit(0)): return Lit(0)
        case Mul(Lit(1), other) | Mul(other, Lit(1)): return optimize(other)
        case Add(a, b): return Add(optimize(a), optimize(b))
        case Mul(a, b): return Mul(optimize(a), optimize(b))
        case Neg(a): return Neg(optimize(a))
        case _: return expr
