"""EitherT monad transformer — adds typed errors to any monad F.

``EitherT[F, E, A]`` wraps ``F[Either[E, A]]`` where ``F`` is any monad
with ``.bind()`` and ``.map()``.
...
Example with Option::
    ...
    >>> from _funstruct._either_t import EitherT
